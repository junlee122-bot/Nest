...
- **⚡ 에어컨 전기요금 계산 (`/utility` 기본 탭)**: 종류·냉방 평수·사용시간만으로 한 달 예상 추가요금을 즉시 계산. 한전 누진제(하계/기타·저압/고압·슈퍼유저) 전후 차이 기반의 **결정론적 순수 함수 엔진**(AI 미사용, 네트워크 0회), 낮음·중앙·높음 범위 표시. **내 에어컨 찾기(선택)**: 제품 라벨 촬영(Claude Vision) 또는 모델명 입력 → 네이버 쇼핑 검색으로 실제 제품 확인, 라벨의 냉방 소비전력을 출처 표기와 함께 계산에 반영. 상세: `docs/AIRCON_CALCULATOR.md`
- **💡 공과금 점검 (`/utility` 보조 탭)**: 요금 입력 → 1인 가구 평균 대비 진단 + 절약 팁 (출처 표기)
...
- **🎬 인터랙티브 쇼케이스 (`/showcase`)**: 심사·발표용 데모. 시나리오 5종(수리·계약서·에어컨 요금·장보기·시세) × 4단계 스테퍼, 데이터 접지 카드 12종, 발표자 모드(`?present=1`, 방향키 이동·Esc 종료)
- **💳 주거비 자동분석 (베타)**: 금융결제원 **오픈뱅킹 테스트베드(모의계좌)** 기반 PoC.
  거래내역을 월세·관리비·전기·가스·수도·통신비로 자동 분류·집계. `/money` 라우트로 **격리**되고,
  화면에 "테스트베드(모의데이터)"를 명확히 표기. 테스트베드 **이용 승인 전에도**
  "예시 데이터로 미리보기"로 동일한 분류·집계 흐름을 시연(예시임을 명시, 분류 로직은 실코드).
...
| `NAVER_CLIENT_ID`/`_SECRET` | – | 내 에어컨 찾기 제품 검색 (없으면 검색만 건너뛰고 계산은 정상) |
